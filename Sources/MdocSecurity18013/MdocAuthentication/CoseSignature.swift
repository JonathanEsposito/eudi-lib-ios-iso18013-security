...
extension Cose {
    ...
    public static func makeDetachedCoseSign1(deviceKey: WalletSigningKey, payloadData: Data) throws-> Cose {
        let coseIn = Cose(type: .sign1, algorithm: deviceKey.curve.defaultVerifyAlgorithm.rawValue, payloadData: payloadData)
        let dataToSign = coseIn.signatureStruct!
        let signature = try deviceKey.signature(dataToSign)
        
        // return COSE_SIGN1 struct
        return Cose(type: .sign1, algorithm: deviceKey.curve.defaultVerifyAlgorithm.rawValue, signature: signature)
    }
